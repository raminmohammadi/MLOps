...
This guide provides a comprehensive approach to set up and run your Fake User Simulation Service using the Google Cloud Console. The objective is to test your flight delay prediction service deployed on Google Kubernetes Engine (GKE) by simulating load and triggering autoscaling.
...
We need a base VM instance to install necessary software and configure the environment. This instance will be used to create a custom image.
...
Installing Python, pip, virtual environment, and necessary Python packages ensures the environment is ready for the script execution. Setting up `/home/user_simulator` and its permissions ensures the directory is accessible and ready to store the CSV file and run the script.
...
Creating a custom image from the configured VM allows you to quickly spawn new instances with the same setup, saving time and ensuring consistency.
...
Creating a dedicated service account ensures that the Cloud Function has access to necessary resources, improving security and manageability.
...
These roles ensure the Cloud Function can create and manage VM instances and read from Cloud Storage.
...
The Cloud Function will automate the process of creating VM instances and running the simulation script when a new CSV file is uploaded to the bucket.
...
Ensuring the necessary dependencies are included in the function.

- In `requirements.txt` file for your Cloud Function, specify the necessary dependencies:
  ```plaintext
...
The Cloud Function will automate the creation of a VM instance, running the simulation script, and deleting the VM instance after the script completes.
...
def start_vm(event):
    ...
    ZONE = os.getenv('ZONE').strip()  # Ensure no trailing spaces
...
Uploading a CSV file to the bucket triggers the Cloud Function, which starts the process of creating a VM instance and running the simulation script.

- Go to **Storage** > **Browser** > your bucket and upload a CSV file.

![cloud function trigger](assets/cloud-function-trigger.png)

#### 13. **Monitor the Cloud Function and VM**

Ensures the function and VM are working as expected and that the script is running correctly.
...
Verifies that the script ran successfully and processed the data.

- SSH into the VM instance created by the Cloud Function and check the `simulation.log` file located in `/home/user_simulator`.

#### 15. **Monitor GKE Autoscaling**

To ensure that the service is handling the load correctly and that autoscaling is functioning as expected.
...
- Check the **Logs** tab to see the incoming requests and responses.
